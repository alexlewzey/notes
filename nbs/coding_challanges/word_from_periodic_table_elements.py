"""Script that receives a word and returns all the combinations of periodic table
elements that can be combined to spell that word, if any exist."""
...
def find_all_matching_element_combinations(
    ...
    if char_1 in elements_lower:
        ...
        find_all_matching_element_combinations(
            word=word, results=results, substring=substring[1:], matches=matches_1
        )
    ...
    if (char_2 in elements_lower) and (len(char_2) == 2):
        ...
        find_all_matching_element_combinations(
            word=word, results=results, substring=substring[2:], matches=matches_2
        )


def main(word: str | None = None) -> bool | None:
