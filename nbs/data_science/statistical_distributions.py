...
def plot_binomial(ns: Any | None = 50, ps: Any | None = 0.5, **kwargs):
...
if p_value < 0.01:
    print(
        f"reject the null hypothesis that the coin unbaised in favor of the alternative hypothesis"
    )
...
plot_stats_dist(
    ...
    dist=stats.nbinom,
    nx=100,
    title="by no. of required successes",
)
plot_stats_dist(
    ns=10,
    ...
    dist=stats.nbinom,
    nx=100,
    title="by probability of success in the bernoilli trials",
)
...
assert dist.cdf(3) == sum([dist.pmf(i) for i in range(4)])
...
data = pd.concat(data)
data["params"] = (
    "loc=" + data["loc"].astype(str) + ", scale=" + data["scale"].astype(str)
)
px.line(
    data, "x", "pdf", color="params", title="normal distribution by parameters"
).plot()
...
stats.dirichlet([100, 600, 200]).pdf(x)
...
def beta_plots():
    ...
    px.line(
        data, "x", "pdf", color="params", title="beta distribution by parameters"
    ).plot()
...
df = (
    pd.Series(dist.pmf(np.arange(dist.ppf(0), dist.ppf(1))))
    .to_frame("pmf")
    .reset_index()
)
